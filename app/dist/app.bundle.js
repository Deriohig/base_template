...
var _font = __webpack_require__(0);
...
var skills = ["write simple, reusable code.", "love React.", "am an expert at Javascript, HTML and CSS.", "am an enterprise design thinker.", "love to teach and learn.", "am fond of a bit of 🍽 and 🍷.", "am seeking a good challenge."].map(function (s) {
  return s + ".";
});
var delay = 2;
var step = 1;
...
function render() {
  ...
  p.innerHTML = $.solid + getRandomColoredString($.prefixP < prefix.length ? Math.min(tail, tail + $.prefixP) : Math.min(tail, skill.length - $.skillP));
...
var Doggo = {
  ...
  getDoggoGallery: function getDoggoGallery() {
...
var elements = document.querySelectorAll(".nav-list li a");
...
function mapPointsToImage(points, element, source) {
  if (points && points.length) {
    points.map(function (point, index) {
      ...
      var mapItem = "<a class='location " + show + "' href='#' data-attr='" + source + "-point-" + index + "' data-x='" + point.xAxis + "%' data-y='" + point.yAxis + "%' style='left: " + point.xAxis + "%; top:" + point.yAxis + "%;' ></a> " + "<div class='info-block col-lg-4 col-md-4 " + flip + "' data-attr='" + source + "-point-" + index + "' style=' " + active + "' > " + " <div class='close-btn'><i class='close rounded'></i></div> <div class='text-area'> <h4> " + point.title + " </h4> <p class='p-14 bold text-brand-primary'>" + point.message + "</p></div></div>";
    ...
    document.querySelectorAll(".component-map-banner .info-block").forEach(function (elem, index) {
      ...
      var dataInfo = elem.getAttribute("data-attr");
      //use the data to get the matching anchor
      var selector = ".component-map-banner a.location[data-attr='" + dataInfo + "']";
      //jquery reference the selector
      var anchor = document.querySelector(selector);
      //get the x and y cordinates of the anchor
      var topPos = anchor.getAttribute("data-y");
      var leftPos = anchor.getAttribute("data-x");
